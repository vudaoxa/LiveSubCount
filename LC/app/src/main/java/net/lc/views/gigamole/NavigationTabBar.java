...
public class NavigationTabBar extends View implements ViewPager.OnPageChangeListener {
    ...
    private final static float RATE_SCALED_BADGE_SIZE = 1.3F;
    private final static float RATE_SCALED_BADGE_BOUND_SIZE = 0.8F;
    private final static int INDICATOR_HEIGHT = 6;
    ...
    private final RectF mIndicatorBounds = new RectF();
...
//                    mBadgeTitleSize = (side * TITLE_SIZE_FRACTION) * BADGE_TITLE_SIZE_FRACTION*RATE_SCALED_BADGE_SIZE;
                mBadgeTitleSize = mModelTitleSize * BADGE_TITLE_SIZE_FRACTION * RATE_SCALED_BADGE_SIZE;
                ...
                mBadgeMargin = (badgeBounds.height() * RATE_SCALED_BADGE_BOUND_SIZE) +
...
//                            DEFAULT_ICON_SIZE_FRACTION : mIconSizeFraction));
    ...
    protected void onDraw(final Canvas canvas) {
        ...
        if (mIsHorizontalOrientation) {
            ...
            float bottom = mBounds.height() + barBadgeMargin;
            mIndicatorBounds.set(
                    mPointerLeftTop, bottom - INDICATOR_HEIGHT,
                    mPointerRightBottom, bottom
            );
        } else mPointerBounds.set(0.0F, mPointerLeftTop, mBounds.width(), mPointerRightBottom);

        // Draw pointer for model colors
        if (mCornersRadius == 0) mPointerCanvas.drawRect(mIndicatorBounds, mPaint);
        else mPointerCanvas.drawRoundRect(mIndicatorBounds, mCornersRadius, mCornersRadius, mPaint);
        ...
        for (int i = 0; i < mModels.size(); i++) {
            ...
            float x = 0.25f;
            float d = mBgBadgeBounds.bottom - mBgBadgeBounds.top;
            float oldTop = mBgBadgeBounds.top;
            float z = 0;
            if (mBadgeGravity == BadgeGravity.BOTTOM) {
                mBgBadgeBounds.top = mBgBadgeBounds.top * x;
                mBgBadgeBounds.bottom = d + mBgBadgeBounds.top;
                z = mBgBadgeBounds.top - oldTop;
            }
            ...
            canvas.drawText(
                    model.getBadgeTitle(), badgeBoundsHorizontalOffset, badgeVerticalOffset +
                            mBadgeBounds.height() - (mBadgeBounds.height() * model.mBadgeFraction) + z,
                    mBadgePaint
            );
        }
    }
